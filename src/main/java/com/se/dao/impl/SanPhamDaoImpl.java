...
public class SanPhamDaoImpl implements SanPhamDao{
	...
	public List<SanPham> getByFilter(String tenLoai, String tenMon, String tenThuongHieu, double fromPrice,
		...
		try {
			...
			String sql = "select SanPham.maSanPham, SanPham.chietKhau, sanPham.trangThai,SanPham.giaTien , SanPham.mieuTa, SanPham.tenSanPham,  SanPham.maLoaiSanPham, SanPham.maMonTheThao, SanPham.maNhanHieu  from SanPham join LoaiSanPham on SanPham.maLoaiSanPham = LoaiSanPham.maLoaiSanPham\r\n"
					...
					+ "						where LoaiSanPham.tenLoaiSanPham like N'%"+tenLoai+"%' and NhanHieu.tenNhanHieu like N'%"+tenThuongHieu
					+"%' and MonTheThao.tenMonTheThao like N'%"+tenMon+"%' and (SanPham.giaTien >="+fromPrice
					+" and SanPham.giaTien<="+toPrice+") 	group by SanPham.maSanPham,sanPham.trangThai ,SanPham.chietKhau, SanPham.giaTien , SanPham.mieuTa, SanPham.tenSanPham,  SanPham.maLoaiSanPham, SanPham.maMonTheThao, SanPham.maNhanHieu  order by SanPham.maSanPham offset "+numPage+" rows  fetch next "+limit+" rows only";
			List<SanPham> list = session.createNativeQuery(sql, SanPham.class).getResultList();
			return list;
	...
	public List<SanPham> getSanPhamFilter(String strLoaiSanPham, String strMonTheThao, String strNhanHieu, double price_to,
		...
		try {
			...
			String sql = "select SanPham.maSanPham, SanPham.chietKhau, sanPham.trangThai,SanPham.giaTien , SanPham.mieuTa, SanPham.tenSanPham,  SanPham.maLoaiSanPham, SanPham.maMonTheThao, SanPham.maNhanHieu  from SanPham join LoaiSanPham on SanPham.maLoaiSanPham = LoaiSanPham.maLoaiSanPham\r\n"
					...
					+ "						join ChiTietSanPham on SanPham.maSanPham = ChiTietSanPham.maSanPham\r\n"
					+ "						where "
					+ strLoaiSanPham + "and" + strMonTheThao + "and"+ strNhanHieu + "and (SanPham.giaTien >="+price_from+" and SanPham.giaTien<="+price_to+") "
	...
	public List<SanPham> getSanPhamByLoaiSanPham(String loaiSanPham) {
		Session session = sessionFactory.getCurrentSession();
		try {
			String sql = "select top 10 * from SanPham join LoaiSanPham on SanPham.maLoaiSanPham = LoaiSanPham.maLoaiSanPham where tenLoaiSanPham like N'"+loaiSanPham+"' order by SanPham.maSanPham desc";
