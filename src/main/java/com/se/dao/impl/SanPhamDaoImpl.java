...
public class SanPhamDaoImpl implements SanPhamDao{
	...
	public List<SanPham> getByFilter(String tenLoai, String tenMon, String tenThuongHieu, double fromPrice,
		...
		try {
			...
			String sql = "select * from SanPham join LoaiSanPham on SanPham.maLoaiSanPham = LoaiSanPham.maLoaiSanPham\r\n"
					...
					+ "						where LoaiSanPham.tenLoaiSanPham like N'%"+tenLoai+"%' and NhanHieu.tenNhanHieu like N'%"+tenThuongHieu+"%' and MonTheThao.tenMonTheThao like N'%"+tenMon+"%' and (SanPham.giaTien >="+fromPrice+" and SanPham.giaTien<="+toPrice+") order by SanPham.maSanPham offset "+numPage+" rows  fetch next "+limit+" rows only";
			List<SanPham> list = session.createNativeQuery(sql, SanPham.class).getResultList();
			return list;
